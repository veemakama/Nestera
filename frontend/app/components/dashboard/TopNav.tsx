...
import { Search, Bell, HelpCircle, Flag } from "lucide-react";
...
const TopNav: React.FC = () => {
  const [showFlagAdmin, setShowFlagAdmin] = useState(false);
  ...
  return (
    <>
        ...
        <div className="flex items-center ml-auto" style={{ gap: 10 }}>
          ...
          {isDev && (
            <button
              ...
              onClick={() => setShowFlagAdmin(true)}
              className="flex items-center justify-center text-[#6a9fae] cursor-pointer hover:text-[#08c1c1] transition-colors bg-[#0e2330] border border-white/8 rounded-xl"
              style={{ width: 38, height: 38 }}
            >
              <Flag size={16} />
            </button>
      ...
      {showFlagAdmin && <FeatureFlagAdmin onClose={() => setShowFlagAdmin(false)} />}
    </>
  );
};
